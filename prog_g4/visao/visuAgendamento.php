...
if(isset($_GET['idAgendamento'])){  
    $cControle = new ControleAgendamento();
    $cControle->excluir($_GET['idAgendamento']);
}
if(isset($_GET['id'])){
    $cControle = new ControleAgendamento();
...
<div class='form-group'>
    <body>
        <div class="row">
            <div class="col-9"> 
            <h1> Visualização do Agendamento de Consulta <a href='edicaoAgendamento.php?id=".$agendamento->getIdAgendamento()."'><img src='/prog_img/editar.png'  height='30' width='30'></a> </h1>
            </div>
        </div>
        <div class="row">
      <div class="col">
        <label class= "control-label" style="margin-right: 10px; margin-left: 10px; margin-top: 5px;">Nome do pet:</label>
        <label class= "control-label" style="margin-right: 10px; margin-left: 10px; margin-top: 15px;">Nome do Tutor:</label>
        <label class= "control-label" style="margin-right: 40px; margin-left: 10px; margin-top: 15px;">E-mail:</label>
        <label class= "control-label" style="margin-right: 40px; margin-left: 10px; margin-top: 15px;">Telefone:</label>
        <label class= "control-label" style="margin-right: 90px; margin-left: 10px; margin-top: 15px;">Data:</label>
        <label class= "control-label" style="margin-right: 50px; margin-left: 10px; margin-top: 15px;">Horário:</label>
        <label class= "control-label" style="margin-right: 50px; margin-left: 10px; margin-top: 15px;">Veterinário:</label>
        <label class= "control-label" style="margin-right: 50px; margin-left: 10px; margin-top: 15px;">Situação:</label>
      </div>

      <div class="col-10">
      	<?php
      	echo "<label type='text' class='form-control mr-sm-2' readonly>".$agendamento->getNomePet()."</label>";
      	echo "<label type='text' class='form-control mr-sm-2' readonly>".$agendamento->getNomeTutor()."</label>";
      	echo "<label type='text' class='form-control mr-sm-2' readonly>".$agendamento->getEmail()."</label>";
      	echo "<label type='text' class='form-control mr-sm-2' readonly>".$agendamento->getTelefone()."</label>";
      	echo "<label type='text' class='form-control mr-sm-2' readonly>".$agendamento->getDate()."</label>";
      	echo "<label type='text' class='form-control mr-sm-2' readonly>".$agendamento->getHora()."</label>";
      	echo "<label type='text' class='form-control mr-sm-2' readonly>".$agendamento->getNomeVeterinario()."</label>";
      	if($agendamento->getStatus() == 0){
                        echo "<label type='text' class='form-control mr-sm-2' readonly>Ativo</label>";
                    }
                    else if($agendamento->getStatus() == 1){
                        echo "<label type='text' class='form-control mr-sm-2' readonly>Não Efetivado</label>";
                    }
                    else if($agendamento->getStatus() == 2){
                        echo "<label type='text' class='form-control mr-sm-2' readonly>Efetivado</label>";
                    }
      	echo "<input type='hidden' name='id' value=".$agendamento->getIdAgendamento().">";
      	echo "</div>";
		echo "</div>";
        echo "<a class='btn btn-primary' href='visuAgendamento.php?idAgendamento=".$agendamento->getIdAgendamento()."'>Excluir</a>";
        echo "   ";
        echo "<a class='btn btn-primary' href='../visao/lstAgendamentos.php'>Voltar</a>";
		?>
    </div>
</div>
</body>
