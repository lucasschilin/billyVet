...
if(isset($_POST['botao']) && $_POST['botao']=="Cadastrar"){
	...
	$cControle->inserir($_POST,$_SESSION['usuario']);
}

if(isset($_POST['voltar']) && $_POST['voltar']=="Voltar à Internação"){
	include_once $_SERVER['DOCUMENT_ROOT']."/prog_g8/desenv/ControleRegistro.class.php";
	$cControle = new ControleRegistro();
	$cControle->voltarInternacao($_POST);
}
...
confirma = confirm("Tem certeza que deseja sair?");
	...
	<head>
		<?php 
			echo "<title>Cadastrar registro de ".$infos['pet']."</title>";
		?>
	...
	<div class="container-fluid">
		...
		<form action="index.php" method="POST">
			<div class="form-group">
  				...
  				<div>
  					<div class="row">
    					...
    					<div class="col-9 espacamento">
							<textarea style="margin-left:2%" name="procedimento" id="idProcedimento" class="form-control noresize" rows="4"></textarea>
    					</div>
    				</div>
	  				...
	  				<div class="row">
    					...
    					<div class="col-9 espacamento">
    						<input class="form-control" style="margin-left:2%" size="145%" type="text" id="idMedicamento" name= "medicamento">
    					</div>
    				</div>
  				 	...
  				 	<div class="row">
    					...
    					<div class="col-9 espacamento">
    						<input class="form-control" style="margin-left:2%" size="145%" type="text" id="idReacoes" name= "reacao">
  						</div>
  					</div>
					...
					<div class="container-fluid">
  					...
  					<div class="row">
						...
						<div class="col-9 espacamento">
							<input class="form-control" style="margin-left:2%" size="145%" type="datetime-local" id="idDTH" name="dt_procedimento" required value="<?=date('Y-m-d H:i:s')?>" max="<?=date('Y-m-d\TH:i')?>" min="2018-11-10T00:00:00" required>
						</div>
  					</div>

  					<div class="row">
 						<div style="margin-left: 30px" class="col-2 espacamento">
 							<label class="form-check-label" for="idVisibilidade">Visível</label>
 							<input class="form-check-input" style="margin-left:2%" size="145%" type="checkbox"  id="idVisibilidade" name="visibilidade" checked value=1>
    					</div>
  					</div><br>
 	 		...
 	 		<?php 
  				echo "<a href='listaRegistros.php?id=".$_GET['id']."' class='btn btn-primary' style='color:white'>Voltar</a>";
  			?>
 	 	</form>
 	</div>
</html>