...
$registros = $cRegistro->listarPorInternacao($_GET['id_internacao']);
...
function alertaRegistro() {
	confirma = confirm("Registro indisponível, contate nossa equipe");
}
</script>
...
<div class="col-8">
	...
	<table class="table table-hover" id=id_inter>
      ...
      <tbody>
        ...
        <tr>
          <?php 
		  	echo "<td>".$registro->getdt_procedimento()."</td>"; 
            echo "<td>".$registro->getProcedimento()."</td>";
			if($registro->getVisibilidade() == 1){
			echo "<td><a href='lstUmRegistro.php?id_registro=".$registro->getid_registro()."'><img src='http://192.168.103.223/prog_img/verum.png' style='width:15px'></a></td>";
			} else{
			echo "<td><a href='#' &acao='registro' onclick='return alertaRegistro();' ><img src='http://192.168.103.223/prog_img/verum.png' style='width:15px'></a></td>";
}
       ...
       ?>
		 ...
		 <a class="btn btn-primary" href="lstInternacoes.php" role="button">Voltar</a>
    </div>
	</div>
